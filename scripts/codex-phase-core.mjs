...
export function parsePrompt(filename, text) {
  ...
  const browserRequiredFields = [
    ...text.matchAll(/^- Browser required:\s*(.*)$/gm),
  ];
  if (browserRequiredFields.length > 1) {
    throw new Error(
      `Expected at most one Browser required field; found ${browserRequiredFields.length}.`,
    );
  }
  let browserRequired = false;
  if (browserRequiredFields.length === 1) {
    const value = browserRequiredFields[0][1].trim();
    if (value === 'yes.') browserRequired = true;
    else if (value !== 'no.') {
      throw new Error(
        `Browser required must be exactly yes. or no.: ${filename}`,
      );
    }
  }
...
function stateLabel(prompt, state) {
  if (prompt.kind === 'closeout') {
    if (state?.status === 'browser_required')
      return '[B] BROWSER REQUIRED / CLOSEOUT';
  ...
  if (state?.status === 'browser_required') return '[B] BROWSER REQUIRED';
...
export function renderFailureSummary({ plan, states, failedPrompt, reason }) {
  ...
  const browserBlockedPrompt = plan?.prompts.find(
    (prompt) => states.get(prompt.number)?.status === 'browser_required',
  );
  if (browserBlockedPrompt)
    lines.push(
      `[B] P${browserBlockedPrompt.number} - ${browserBlockedPrompt.title} - BROWSER EXECUTION REQUIRED`,
      '',
    );
  ...
  else if (plan?.closeout && closeoutState?.status === 'browser_required')
    lines.push(
      `  [B] P${plan.closeout.number} - ${plan.closeout.title} - BROWSER EXECUTION REQUIRED`,
    );
