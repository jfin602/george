...
function durableEvent(event: ApplicationEvent): ApplicationEvent | undefined {
  switch (event.type) {
    ...
    case 'context.envelope.promoted': return {
      type: event.type, turnId: string(event.turnId, 'turn ID', 256), fromProfileId: string(event.fromProfileId, 'source context profile ID', 256), toProfileId: string(event.toProfileId, 'target context profile ID', 256),
      reason: oneOf(event.reason, 'context envelope promotion reason', ['continuation-estimate', 'provider-usage']), tokens: boundedInteger(event.tokens, 'context envelope promotion tokens'), providerInputBudget: boundedInteger(event.providerInputBudget, 'context envelope provider input budget'),
    };
...
function safeCheckpoint(value: ContextCheckpointEvidence): ContextCheckpointEvidence {
  ...
  const reason = oneOf(checkpoint.reason, 'context checkpoint reason', ['soft-pressure', 'hard-pressure', 'stall-pressure']);
