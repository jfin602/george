...
export const BENCHMARK_SUITE_VERSION = 'v3';
const DEFAULT_REPETITIONS = 1;

export type BenchmarkSuiteName = 'quick' | 'full' | 'context' | 'agentic-context';
export type BenchmarkCategory = 'short-reasoning' | 'long-context-retrieval' | 'code-understanding' | 'structured-tool-use' | 'independent-multi-tool-inspection' | 'repository-investigation' | 'coding-edit-workflow' | 'repository-repair-workflow' | 'extended-agent-loop' | 'compaction-noisy-evidence' | 'agentic-repository-inspection' | 'agentic-multi-round-investigation' | 'agentic-edit-validation';
export type BenchmarkOptions = Readonly<{ suite: BenchmarkSuiteName; model?: string; baseUrl?: string; repetitions: number; label?: string; compare?: string; artifactRoot?: string; contextBands?: readonly number[]; agenticContextBands?: readonly number[] }>;
export type BenchmarkCase = Readonly<{ id: string; version: number; category: BenchmarkCategory; suites: readonly BenchmarkSuiteName[]; contextProfileId?: string; fixture: 'none' | 'code' | 'tools' | 'investigation' | 'edit' | 'repository-workflow' | 'extended' | 'agentic-inspection' | 'agentic-investigation' | 'agentic-edit'; input: (band?: number) => string; expected: Readonly<{ answer?: string; requiredText?: string; tools?: readonly string[]; allowedTools?: readonly string[]; toolCallRange?: readonly [number, number]; edit?: Readonly<{ path: string; content: string }>; validation?: Readonly<{ executable: string; arguments: readonly string[] }> }>; band?: number; routedDocuments?: readonly string[]; history?: string }>;
export type BenchmarkRecord = Readonly<{
  ...
  model: string; provider: Readonly<{ type: 'lm-studio-responses'; origin: string }>; contextProfileId?: string; requestedContextBand?: number; estimatedContextTokens?: number; contextSelection?: Readonly<{ mode: 'adaptive' | 'fixed'; selectedProfile: Readonly<{ id: string; providerInputTokens: number; reservedHeadroomTokens: number }>; attemptedProfileIds: readonly string[]; promotionReasons: readonly string[]; providerInputBudget: number; remainingHeadroom: number; categoryTokens: Readonly<Record<string, number>>; dispositions: Readonly<Record<string, number>> }>; actualInputTokens?: number; actualOutputTokens?: number;
...
export const DEFAULT_AGENTIC_CONTEXT_BANDS = [2048, 4096, 6144, 8192, 10240, 12288] as const;
...
const agenticToken = (band: number, family: string): string => `AGENTIC_${family.toUpperCase()}_${band}_ORCHID`;
const agenticInput = (band: number, family: string, task: string): string => `${task}\nFollow the applicable repository guidance. Return exactly ${agenticToken(band, family)} after completing the required repository work.`;
const agenticCase = (band: number, family: 'inspection' | 'investigation' | 'edit'): BenchmarkCase => {
  const common = { version: 1, suites: ['agentic-context'] as const, band, routedDocuments: ['docs/task-brief.md', 'docs/evidence-notes.md'], history: 'Earlier completed work established that this local benchmark has no network dependency. The current repository fact must still be read from the named artifact.' };
  if (family === 'inspection') return { ...common, id: `agentic-inspection-${band}-001`, category: 'agentic-repository-inspection', fixture: 'agentic-inspection', input: () => agenticInput(band, family, 'Read docs/ownership.md exactly once to establish the repository fact.'), expected: { answer: agenticToken(band, family), tools: ['read_file'], allowedTools: ['read_file'], toolCallRange: [1, 1] } };
  if (family === 'investigation') return { ...common, id: `agentic-investigation-${band}-001`, category: 'agentic-multi-round-investigation', fixture: 'agentic-investigation', input: () => agenticInput(band, family, 'Start with investigation/entry.txt and read each next artifact it names. Do not infer missing evidence.'), expected: { answer: agenticToken(band, family), tools: ['read_file'], allowedTools: ['read_file'], toolCallRange: [4, 4] } };
  return { ...common, id: `agentic-edit-validation-${band}-001`, category: 'agentic-edit-validation', fixture: 'agentic-edit', input: () => agenticInput(band, family, 'Read src/label.js, repair the deterministic defect with one write, and rely on the required validation command.'), expected: { answer: agenticToken(band, family), tools: ['read_file', 'write_file', 'run_process'], allowedTools: ['read_file', 'write_file', 'run_process'], toolCallRange: [3, 3], edit: { path: 'src/label.js', content: "export const label = (value) => value.trim().toUpperCase();\n" }, validation: { executable: 'node', arguments: ['--test', 'test/label.test.js'] } } };
};

/** A real application run per band, ordered from read-only inspection through a bounded repair. */
export function agenticContextCases(bands: readonly number[] = DEFAULT_AGENTIC_CONTEXT_BANDS): readonly BenchmarkCase[] {
  return [...bands].sort((left, right) => left - right).flatMap((band) => [agenticCase(band, 'inspection'), agenticCase(band, 'investigation'), agenticCase(band, 'edit')]);
}
...
export function parseBenchmarkArguments(argv: readonly string[]): BenchmarkOptions | Readonly<{ help: true }> {
  const result: { suite: BenchmarkSuiteName; model?: string; baseUrl?: string; repetitions: number; label?: string; compare?: string; artifactRoot?: string; contextBands?: readonly number[]; agenticContextBands?: readonly number[] } = { suite: 'quick', repetitions: DEFAULT_REPETITIONS };
  for (let index = 0; index < argv.length; index += 1) {
    const argument = argv[index]!;
    if (argument === '--help' || argument === '-h') return { help: true };
    if (!['--suite', '--model', '--base-url', '--repetitions', '--label', '--compare', '--artifacts', '--context-bands', '--agentic-context-bands'].includes(argument)) throw new Error(`Unknown benchmark flag: ${argument}`);
    const value = argv[++index];
    if (value === undefined || value.startsWith('--')) throw new Error(`Missing value for ${argument}.`);
    if (argument === '--suite') { if (value !== 'quick' && value !== 'full' && value !== 'context' && value !== 'agentic-context') throw new Error('Suite must be quick, full, context, or agentic-context.'); result.suite = value; }
    ...
    if (argument === '--context-bands' || argument === '--agentic-context-bands') {
      const label = argument === '--context-bands' ? 'Context bands' : 'Agentic context bands';
      if (!/^\d+(?:,\d+)*$/.test(value)) throw new Error(`${label} must be comma-separated positive integers.`);
      const bands = value.split(',').map(Number);
      if (bands.some((band) => band < 1 || band > MAX_CONTEXT_BAND)) throw new Error(`${label} must be integers from 1 to ${MAX_CONTEXT_BAND}.`);
      if (new Set(bands).size !== bands.length) throw new Error(`${label} must not contain duplicates.`);
      if (argument === '--context-bands') result.contextBands = bands.toSorted((left, right) => left - right);
      else result.agenticContextBands = bands.toSorted((left, right) => left - right);
    }
  }
  if (result.contextBands !== undefined && result.suite !== 'context') throw new Error('Context bands require --suite context.');
  if (result.agenticContextBands !== undefined && result.suite !== 'agentic-context') throw new Error('Agentic context bands require --suite agentic-context.');
  return result;
}

export function benchmarkUsage(): string { return 'Usage: npm run benchmark -- [--suite quick|full|context|agentic-context] [--context-bands 2048,4096,8192,16384,24576] [--agentic-context-bands 2048,4096,6144,8192,10240,12288] [--model ID] [--base-url LOOPBACK_URL] [--repetitions 1..20] [--label NAME] [--compare results.json]'; }
export function casesFor(suite: BenchmarkSuiteName, contextBands?: readonly number[], agenticContextBands?: readonly number[]): readonly BenchmarkCase[] { return suite === 'context' ? contextLadderCases(contextBands) : suite === 'agentic-context' ? agenticContextCases(agenticContextBands) : BENCHMARK_CASES.filter((item) => item.suites.includes(suite)); }

function agenticGuidance(band: number, source: string): string {
  const records = Math.max(5, Math.ceil((band - 900) / 216));
  return [
    `# ${source} benchmark guidance`,
    'Treat repository files as evidence, keep changes bounded, and do not invoke network-backed tools for this local fixture.',
    ...Array.from({ length: records }, (_, index) => `Work item ${index + 1}: component ${source.toLowerCase().replaceAll(' ', '-')}-${index + 1} owns a compact local contract. Preserve explicit inputs, inspect the named evidence before deciding, record validation results, and defer unrelated maintenance.`),
  ].join('\n');
}

async function addAgenticContext(root: string, case_: BenchmarkCase): Promise<void> {
  const band = case_.band ?? 2048;
  await Promise.all([mkdir(join(root, '.george')), mkdir(join(root, 'docs'))]);
  await Promise.all([
    writeFile(join(root, '.george', 'instructions.md'), agenticGuidance(band, 'Local workflow')),
    writeFile(join(root, 'AGENTS.md'), agenticGuidance(band, 'Repository ownership')),
    writeFile(join(root, 'docs', 'task-brief.md'), agenticGuidance(band, 'Task brief')),
    writeFile(join(root, 'docs', 'evidence-notes.md'), agenticGuidance(band, 'Evidence notes')),
  ]);
}

export async function createBenchmarkFixture(kind: BenchmarkCase['fixture'], case_?: BenchmarkCase): Promise<string> {
  ...
  if (kind === 'agentic-inspection' || kind === 'agentic-investigation' || kind === 'agentic-edit') {
    if (!case_) throw new Error('Agentic benchmark fixtures require their case definition.');
    await addAgenticContext(root, case_);
  }
  if (kind === 'agentic-inspection') await writeFile(join(root, 'docs', 'ownership.md'), `The architecture authority is ${case_!.expected.answer}.\n`);
  if (kind === 'agentic-investigation') { await mkdir(join(root, 'investigation')); await Promise.all([writeFile(join(root, 'investigation', 'entry.txt'), 'Read investigation/module-map.txt next.\n'), writeFile(join(root, 'investigation', 'module-map.txt'), 'Read investigation/boundary.txt next.\n'), writeFile(join(root, 'investigation', 'boundary.txt'), 'Read investigation/evidence.txt next.\n'), writeFile(join(root, 'investigation', 'evidence.txt'), `${case_!.expected.answer}\n`)]); }
  if (kind === 'agentic-edit') { await Promise.all([mkdir(join(root, 'src')), mkdir(join(root, 'test'))]); await Promise.all([writeFile(join(root, 'package.json'), '{"type":"module"}\n'), writeFile(join(root, 'src', 'label.js'), 'export const label = (value) => value.trim();\n'), writeFile(join(root, 'test', 'label.test.js'), "import assert from 'node:assert/strict';\nimport test from 'node:test';\nimport { label } from '../src/label.js';\n\ntest('label normalizes input', () => assert.equal(label(' george '), 'GEORGE'));\n")]); }
...
export function deriveBenchmarkRecord(case_: BenchmarkCase, events: readonly ApplicationEvent[], elapsedMs: number, repetition: number, execution: BenchmarkRecord['execution'], model: string, origin: string, label?: string, timestamps?: readonly number[]): BenchmarkRecord {
  let estimate: number | undefined; let selection: BenchmarkRecord['contextSelection']; let input = 0; let output = 0; let hasInput = false; let hasOutput = false; let attemptStarted: number | undefined; let responseStart: number | undefined; let firstOutput: number | undefined;
  ...
  for (const [index, event] of events.entries()) {
    ...
    if (event.type === 'context.assembled') {
      estimate = event.diagnostics.estimatedTokens;
      const diagnostics = event.diagnostics as Partial<typeof event.diagnostics>;
      const dispositions = Object.fromEntries(['omitted', 'deferred', 'duplicate', 'failed', 'routed'].map((disposition) => [disposition, diagnostics.evidence?.filter((item) => item.disposition === disposition).length ?? 0]));
      if (diagnostics.mode && diagnostics.profileId && diagnostics.profile && diagnostics.attemptedProfileIds && diagnostics.promotionReasons && diagnostics.providerInputBudget !== undefined && diagnostics.remainingHeadroom !== undefined && diagnostics.categoryTokens) selection = { mode: diagnostics.mode, selectedProfile: { id: diagnostics.profileId, providerInputTokens: diagnostics.profile.providerInputTokens, reservedHeadroomTokens: diagnostics.reservedHeadroom ?? 0 }, attemptedProfileIds: diagnostics.attemptedProfileIds, promotionReasons: diagnostics.promotionReasons, providerInputBudget: diagnostics.providerInputBudget, remainingHeadroom: diagnostics.remainingHeadroom, categoryTokens: diagnostics.categoryTokens, dispositions };
    }
  ...
  const expectedTools = case_.expected.tools ?? []; const unexpectedTools = (case_.expected.allowedTools === undefined ? [] : tools.filter((tool) => !case_.expected.allowedTools!.includes(tool))); const toolsOk = expectedTools.every((tool) => tools.includes(tool)) && unexpectedTools.length === 0; const toolCalls = events.filter((event) => event.type === 'tool.requested').length;
  ...
  const failureReason = !terminalOk ? terminal?.type === 'turn.failed' ? terminal.error.message : 'Turn did not complete.' : unexpectedTools.length ? `Unexpected tools: ${unexpectedTools.join(', ')}.` : !toolsOk ? `Missing required tools: ${expectedTools.filter((tool) => !tools.includes(tool)).join(', ')}.` : !callCountOk ? `Expected ${minimumTools}-${maximumTools} tool calls; observed ${toolCalls}.` : !answerOk ? case_.expected.requiredText === undefined ? `Expected exact answer ${case_.expected.answer}.` : `Missing required fact: ${case_.expected.requiredText}.` : undefined;
  const memory = process.memoryUsage();
  return { suiteVersion: BENCHMARK_SUITE_VERSION, caseId: case_.id, caseVersion: case_.version, category: case_.category, ...(label === undefined ? {} : { label }), timestamp: new Date().toISOString(), repetition, execution, model, provider: { type: 'lm-studio-responses', origin }, contextProfileId: selection?.selectedProfile.id ?? case_.contextProfileId ?? DEFAULT_CONTEXT_PROFILE.id, ...(case_.band === undefined ? {} : { requestedContextBand: case_.band }), ...(estimate === undefined ? {} : { estimatedContextTokens: estimate }), ...(selection === undefined ? {} : { contextSelection: selection }), ...(hasInput ? { actualInputTokens: input } : {}), ...(hasOutput ? { actualOutputTokens: output } : {}), ...(attemptStarted === undefined || responseStart === undefined ? {} : { responseStartLatencyMs: Math.round(responseStart - attemptStarted) }), ...(attemptStarted === undefined || firstOutput === undefined ? {} : { firstOutputLatencyMs: Math.round(firstOutput - attemptStarted) }), elapsedMs: measuredElapsedMs, providerActiveMs, providerRoundActiveMs, averageProviderRoundMs: rounds === 0 ? 0 : Math.round(providerRoundActiveMs / rounds), providerActivePercent: percent(providerActiveMs, measuredElapsedMs), toolExecutionMs, toolExecutionPercent: percent(toolExecutionMs, measuredElapsedMs), approvalWaitMs, approvalWaitPercent: percent(approvalWaitMs, measuredElapsedMs), otherMs, otherPercent: percent(otherMs, measuredElapsedMs), providerAttempts: attempts, providerRounds: rounds, retries, toolCalls, uniqueTools: tools, validationCount: validations, rssBytes: memory.rss, heapUsedBytes: memory.heapUsed, passed: failureReason === undefined, ...(failureReason === undefined ? {} : { failureReason }), notes: ['providerActiveMs measures George-observed provider attempt lifecycles, not GPU inference time.', 'otherMs excludes the union of observed provider, tool, and approval intervals.', 'execution labels process order; it does not claim model residency control.'] };
}

export async function runBenchmarkCase(case_: BenchmarkCase, provider: ModelProvider, options: BenchmarkOptions, repetition: number, execution: BenchmarkRecord['execution'], model: string, origin: string): Promise<BenchmarkRecord> {
  const root = await createBenchmarkFixture(case_.fixture, case_); const state = await mkdtemp(join(tmpdir(), 'george-benchmark-config-')); const events: ApplicationEvent[] = []; const timestamps: number[] = [];
  try {
    const validation = validationFor(case_);
    const approval = createBenchmarkApproval(case_);
    const agentic = case_.suites.includes('agentic-context');
    const serviceOptions = { provider, workspace: root, userConfigRoot: state, ...(agentic ? {} : { toolNames: case_.fixture === 'none' ? [] : case_.fixture === 'code' || case_.fixture === 'tools' || case_.fixture === 'investigation' ? ['read_file'] : ['read_file', 'write_file', 'run_process'], parallelSearch: false as const }), approvalPort: approval, mcp: false as const, chromeDevtools: false as const };
    const session = createSession({ workspace: root });
    if (case_.history) session.transcript.push({ role: 'user', text: case_.history }, { role: 'assistant', text: 'Acknowledged: inspect current repository evidence before answering.' });
    const started = now();
    if (case_.fixture === 'edit' || case_.fixture === 'repository-workflow' || case_.fixture === 'extended' || case_.fixture === 'agentic-edit') {
      const service = await createCodingWorkflowApplicationService(serviceOptions);
      await service.run({ session, input: case_.input(case_.band), ...(case_.routedDocuments === undefined ? {} : { routedDocuments: case_.routedDocuments }), validations: [{ label: 'fixture validation', intent: 'Verify the bounded edit.', executable: validation.executable, arguments: [...validation.arguments] }], onEvent: (event) => { events.push(event); timestamps.push(now() - started); } });
    } else {
      const service = await createAgentLoopApplicationService(serviceOptions);
      for await (const event of service.run({ session, input: case_.input(case_.band), ...(case_.routedDocuments === undefined ? {} : { routedDocuments: case_.routedDocuments }) })) { events.push(event); timestamps.push(now() - started); }
    }
...
export async function runBenchmark(options: BenchmarkOptions, observer?: BenchmarkProgressObserver): Promise<BenchmarkResults> {
  ...
  const cases = casesFor(options.suite, options.contextBands, options.agenticContextBands); const totalExecutions = cases.length * options.repetitions;
...
export async function compareBenchmark(path: string, current: BenchmarkResults): Promise<string> { const previous = JSON.parse(await readFile(path, 'utf8')) as { schemaVersion?: number; suiteVersion?: string; records?: readonly BenchmarkRecord[] }; const compatible = (previous.schemaVersion === 2 && previous.suiteVersion === 'v2') || (previous.schemaVersion === current.schemaVersion && previous.suiteVersion === current.suiteVersion); if (!compatible) throw new Error(`Cannot compare benchmark schema/suite ${previous.schemaVersion ?? 'unknown'}/${previous.suiteVersion ?? 'unknown'} with ${current.schemaVersion}/${current.suiteVersion}.`); const lines = ['# Benchmark comparison', '', ...(previous.schemaVersion === 2 ? ['Legacy v2 artifact: context-selection evidence was not recorded.', ''] : []), '| Case | elapsed ms delta | provider-active ms delta | provider attempts delta | tool calls delta | input/output tokens delta | pass |', '| --- | ---: | ---: | ---: | ---: | ---: | --- |']; for (const record of current.records) { const prior = previous.records?.find((item) => item.caseId === record.caseId && item.repetition === record.repetition); if (prior) lines.push(`| ${record.caseId} | ${record.elapsedMs - prior.elapsedMs} | ${record.providerActiveMs - prior.providerActiveMs} | ${record.providerAttempts - prior.providerAttempts} | ${record.toolCalls - prior.toolCalls} | ${(record.actualInputTokens ?? 0) - (prior.actualInputTokens ?? 0)} / ${(record.actualOutputTokens ?? 0) - (prior.actualOutputTokens ?? 0)} | ${prior.passed} -> ${record.passed} |`); } return `${lines.join('\n')}\n`; }
