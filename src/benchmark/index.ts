...
export function formatBenchmarkSummary(results: BenchmarkResults, elapsedMs: number, directory: string): string {
  ...
  return ['', 'Benchmark complete', '', `Passed:        ${passed} / ${records.length}`, `Elapsed:       ${formatBenchmarkDuration(elapsedMs)}`, `Suite:         ${results.run.suite} ${results.suiteVersion}`, `Model:         ${results.run.provider.model}`, `Artifacts:     ${directory}`, '', `Median case:   ${formatBenchmarkDuration(median(records.map((record) => record.elapsedMs)))}`, `Provider calls: ${timing.providerAttempts}`, `Tool calls:    ${records.reduce((total, record) => total + record.toolCalls, 0)}`, `Retries:       ${timing.retries}`, `Input tokens:  ${inputs.length === 0 ? '-' : number(inputs.reduce((total, value) => total + value, 0))}`, `Output tokens: ${outputs.length === 0 ? '-' : number(outputs.reduce((total, value) => total + value, 0))}`, '', 'Measured case timing', formatTiming(timing)].join('\n');
}
...
export async function runBenchmark(options: BenchmarkOptions, observer?: BenchmarkProgressObserver): Promise<BenchmarkResults> {
  ...
  const cases = casesFor(options.suite); const totalExecutions = cases.length * options.repetitions;
