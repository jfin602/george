...
test('projection safely summarizes every built-in plus truthful denial, interruption, validation, and completion', () => {
  ...
  const calls: ApplicationEvent[] = [
    ...
    { type: 'approval.requested', turnId: 't', callId: 'write', request: { id: 'write', toolName: 'write_file', execution: { effect: 'workspace_mutation', replaySafety: 'not_replay_safe', source: { kind: 'builtin' } }, target: { path: 'a.txt', alreadyDirty: false } } },
    { type: 'approval.denied', turnId: 't', callId: 'write', request: { id: 'write', toolName: 'write_file', execution: { effect: 'workspace_mutation', replaySafety: 'not_replay_safe', source: { kind: 'builtin' } }, target: { path: 'a.txt', alreadyDirty: false } } },
