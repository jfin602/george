...
test('recovery confirms only exact writes, preserves history, and never replays ambiguous work', async (t) => {
  ...
  appendSessionEvent(unknown, { type: 'tool.started', turnId: 'turn', callId: 'process', name: 'run_process', execution: { effect: 'sandboxed_workspace_process', replaySafety: 'not_replay_safe', source: { kind: 'builtin' }, descriptor: { service: 'Bubblewrap', operation: 'workspace process with isolated network' } } });
