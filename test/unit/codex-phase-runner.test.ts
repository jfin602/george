...
const createPhaseRepository = async (
  ...
  const rootDirectory = await mkdtemp(
    path.join(tmpdir(), 'petri-phase-git-test-'),
  );
...
const createCorrectionRepository = async (implementationCount = 2) => {
  const rootDirectory = await mkdtemp(
    path.join(tmpdir(), 'petri-correction-git-test-'),
  );
...
const createPost2PhaseRepository = async (implementationCount = 1) => {
  const rootDirectory = await mkdtemp(
    path.join(tmpdir(), 'petri-post-2-phase-git-test-'),
  );
...
test('GPT-5.6 Codex CLI versions are parsed and compared numerically', () => {
  ...
  assert.deepEqual(parseCodexCliVersion('codex-cli 0.155.0-alpha.2.6'), [0, 155, 0]);
  assert.equal(
...
test('runCodex sends the preflight snapshot and uses native final-response capture', async () => {
  const temporaryDirectory = await mkdtemp(
    path.join(tmpdir(), 'petri-runner-test-'),
  );
  try {
...
test('parse-only validator reports correction identity and unchanged version without writes', async () => {
  ...
  try {
    ...
    assert.match(
      String(result.stdout),
      /P1 \| implementation \| Terra High \| Browser required: no \| 0\.10\.0 \(UNCHANGED\)/,
    );
...
test('parse-only validator identifies browser-required prompts without writes', async () => {
  const rootDirectory = await createPhaseRepository(1);
  const validator = path.resolve('scripts', 'validate-codex-phase.mjs');
  try {
    const promptFile = path.join(rootDirectory, 'docs', 'tasks', 'p8', 'P1-task-1.txt');
    const entry = await readFile(promptFile, 'utf8');
    await writeFile(
      promptFile,
      entry.replace(
    ...
    assert.equal(result.status, 0, String(result.stderr));
    assert.match(String(result.stdout), /P1 \| implementation \| Terra High \| Browser required: yes \|/);
    assert.match(String(result.stdout), /P2 \| closeout \| Terra High \| Browser required: no \|/);
  } finally {
    await rm(rootDirectory, { recursive: true, force: true });
  }
});
...
test('browser-required implementation prompts stop before Codex and preserve the completed prefix', async () => {
  const rootDirectory = await createPhaseRepository(2);
  const calls: number[] = [];
  try {
    const browserPrompt = prompt(2, { browserRequired: 'yes' });
    await writeFile(
      path.join(rootDirectory, 'docs', 'tasks', 'p8', browserPrompt.filename),
      browserPrompt.text,
    );
    gitResult(rootDirectory, ['add', '-A']);
    gitResult(rootDirectory, ['commit', '--quiet', '-m', 'browser metadata']);
    await assert.rejects(
      runCli(['p8'], {
        rootDirectory,
        stdout: testOutput(false),
        resolveLauncher: async () => ({ launcher: directTestLauncher, version: compatibleCodexVersion }),
        runCodexProcess: async (parsedPrompt: { number: number }) => {
          calls.push(parsedPrompt.number);
          await writeFile(path.join(rootDirectory, 'package.json'), `${JSON.stringify({ name: 'phase-test', version: '0.8.1' })}\n`);
          await writeFile(path.join(rootDirectory, 'P1-complete.txt'), 'done\n');
          return { code: 0, signal: null, finalResponse: 'done', stderr: '', childArgs: [] };
        },
      }),
      /P2 requires browser execution\. Run this prompt manually in the Codex app with browser enabled\. Prompt: docs\/tasks\/p8\/P2-task-2\.txt/,
    );
    assert.deepEqual(calls, [1]);
    assert.equal(JSON.parse(await readFile(path.join(rootDirectory, 'package.json'), 'utf8')).version, '0.8.1');
    ...
    const run = JSON.parse(await readFile(path.join(rootDirectory, '.codex-runs', 'p8', runName!, 'run.json'), 'utf8'));
    assert.equal(run.status, 'browser_execution_required');
    assert.equal(run.prompts[0].status, 'passed');
    assert.equal(run.prompts[1].status, 'browser_required');
    assert.equal(run.prompts[1].blockReason, 'browser_execution_required');
    assert.equal(run.prompts[1].commitSha, undefined);
    ...
    await assert.rejects(
      runCli(['p8'], {
        ...
        resolveLauncher: async () => ({ launcher: directTestLauncher, version: compatibleCodexVersion }),
        runCodexProcess: async () => {
          throw new Error('The browser-required prompt must not launch Codex.');
        },
      }),
      /P2 requires browser execution/,
    );
    assert.deepEqual(calls, [1]);
  } finally {
    await rm(rootDirectory, { recursive: true, force: true });
...
test('browser-required closeouts are blocked by --closeout without a Codex invocation', async () => {
  const rootDirectory = await createPhaseRepository(0);
  const calls: number[] = [];
  let launcherResolutions = 0;
  try {
    const closeout = prompt(1, { closeout: true, browserRequired: 'yes' });
    await writeFile(
      path.join(rootDirectory, 'docs', 'tasks', 'p8', closeout.filename),
      closeout.text,
    );
    ...
    await assert.rejects(
      runCli(['p8', '--closeout'], {
        ...
        resolveLauncher: async () => {
          ...
          return { launcher: directTestLauncher, version: compatibleCodexVersion };
        },
        runCodexProcess: async (parsedPrompt: { number: number }) => {
          calls.push(parsedPrompt.number);
          return { code: 0, signal: null, finalResponse: 'done', stderr: '', childArgs: [] };
      ...
      /P1 requires browser execution\. Run this prompt manually in the Codex app with browser enabled/,
    );
    assert.equal(launcherResolutions, 0);
    assert.deepEqual(calls, []);
    assert.equal(gitResult(rootDirectory, ['log', '-1', '--format=%s']), 'browser metadata');
    const [runName] = await readdir(path.join(rootDirectory, '.codex-runs', 'p8'));
    const run = JSON.parse(await readFile(path.join(rootDirectory, '.codex-runs', 'p8', runName!, 'run.json'), 'utf8'));
    assert.equal(run.status, 'browser_execution_required');
    assert.equal(run.prompts[0].status, 'browser_required');
  } finally {
    await rm(rootDirectory, { recursive: true, force: true });
  }
});
