...
test('Petri rejects model configurations below Luna Medium', () => {
...
test('Browser required metadata parses canonically and defaults to no', () => {
  const yes = prompt(1, { browserRequired: 'yes' });
  const no = prompt(1, { browserRequired: 'no' });
  const omitted = prompt(1);

  assert.equal(parsePrompt(yes.filename, yes.text).browserRequired, true);
  assert.equal(parsePrompt(no.filename, no.text).browserRequired, false);
  assert.equal(
    parsePrompt(omitted.filename, omitted.text).browserRequired,
    false,
  );

  for (const value of ['true.', 'Yes.', 'yes', 'maybe.', '']) {
    const malformed = yes.text.replace('Browser required: yes.', `Browser required: ${value}`);
    assert.throws(
      () => parsePrompt(yes.filename, malformed),
      /Browser required must be exactly yes\. or no\./,
    );
  }
  const duplicate = `${yes.text}\n- Browser required: no.\n`;
  assert.throws(
    () => parsePrompt(yes.filename, duplicate),
    /at most one Browser required field; found 2/,
  );
});
